...
在 A/B 两轮全部提交后、**不改任何代码**，用仓库内脚本
`deploy/scripts/bench-round.ps1` 原样再跑一轮（先 postdove，后
mailezine），验证结果可复现：

| 指标 | postdove | mailezine |
|---|---|---|
| 收件吞吐 / p50 | 11.5 msg/s / 1.65 s | **21.6 msg/s / 914 ms** |
| 提交吞吐 / p50 | 3.3 msg/s / 5.31 s | **6.4 msg/s / 2.93 s** |
| IMAP 会话 / FETCH p50 | 100/100 / 21.1 ms | 100/100 / **18.4 ms** |
| 队列 submit / deliver / p50 / backlog | 5.1 / 3.9 msg/s / 20.0 s / 130 | **8.4 / 8.0 msg/s / 3.89 s / 0** |
| 内存空闲（整栈） | ~128 MiB | **~24 MiB** |
| 内存负载峰值 | ~267 MiB | **~34 MiB** |

与 A/B 均值对比，最大偏差：收件 ±20%（postdove 11.5 vs 8.9）、提交
±5%、IMAP p50 ±5%、队列 p50 ±8%、内存 ±2%。方向与比值完全一致
（收件 1.9×、提交 2.0×、IMAP 1.1×、队列 5.2×、内存 5.3×/7.8×），
复现性成立。

> Round C 的 postdove IMAP 灌信阶段（前 100 用户各 1 封）出现一次
> 偶发投递失败（99/100，单连接耗时 70s），属于 postdove 队列在该时刻
> 的瞬时拥堵；IMAP 主测试仍 100/100，不影响结论，记录在案。
