...
type HeaderTest struct {
	...
	Header []string
	Index  int // RFC 5260: 1-based field position to match (0 = all)
}
...
func (h HeaderTest) Check(_ context.Context, d *RuntimeData) (bool, error) {
	...
	for _, hdr := range h.Header {
		...
		if h.Index > 0 {
			if h.Index <= len(values) {
				value := values[h.Index-1]
				if h.isCount() {
					entryCount++
				} else if ok, err := h.matcherTest.tryMatch(d, value); err != nil {
					return false, err
				} else if ok {
					return true, nil
				}
