...
func serveSMTP(ctx context.Context, srv *gosmtp.Server, addr string, maxConn int, proxy bool, proxyTrusted []*net.IPNet, tlsConf *tls.Config, logger *slog.Logger) error {
	...
	// 503. Wrap below the TLS layer: above it the library no longer sees the
	// *tls.Conn it type-asserts and stops advertising AUTH. Replies inside an
	// established TLS session therefore keep 502.
