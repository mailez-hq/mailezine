...
func (c *Conn) availableCaps() []imap.Cap {
	...
	if available.Has(imap.CapIMAP4rev1) {
		caps = append(caps, []imap.Cap{
			...
			imap.CapID, // RFC 2971: no backend support required
		}...)
	}
	if c.canStartTLS() {
		caps = append(caps, imap.CapStartTLS)
	}
	if _, ok := c.session.(SessionExtension); ok {
		// RFC 4314 ACL and RFC 2087 QUOTA are served through the session
		// extension hook.
		caps = append(caps, imap.CapACL, imap.CapQuota)
	}
	if c.canAuth() {
		mechs := []string{"PLAIN"}
	...
	if c.state == imap.ConnStateAuthenticated || c.state == imap.ConnStateSelected {
		...
		addAvailableCaps(&caps, available, []imap.Cap{
			...
			imap.CapCondStore, // RFC 7162
			imap.CapSort,      // RFC 5256
		})
