...
func (c *Conn) handleStore(dec *imapwire.Decoder, numKind NumKind) error {
	...
	// RFC 7162: STORE <set> [(UNCHANGEDSINCE <modseq>)] <item> <flags>
	if dec.Special('(') {
		if !dec.ExpectAtom(&item) || !strings.EqualFold(item, "UNCHANGEDSINCE") ||
			!dec.ExpectSP() || !dec.ExpectModSeq(&unchangedSince) ||
			!dec.ExpectSpecial(')') || !dec.ExpectSP() || !dec.ExpectAtom(&item) {
			return dec.Err()
		}
	} else if !dec.ExpectAtom(&item) {
		return dec.Err()
	}
	if !dec.ExpectSP() {
		return dec.Err()
