// Package rspamd implements the inbound classification stage
...
//
// Scanning ships in both editions. Supervised learning (Learn/LearnWithFuzzy,
// fuzzy hashing) is gated by the controller endpoint: editions that must not
// learn simply build the client with an empty LearnURL, which turns every
// learning call into a no-op.
package rspamd
...
func (c *Client) Classify(ctx context.Context, peer net.IP, from string, to []string, data []byte) (delivery.Result, error) {
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return delivery.Result{}, fmt.Errorf("spam: rspamd status %d", resp.StatusCode)
	}
	var rsp struct {
	...
	if err := json.NewDecoder(resp.Body).Decode(&rsp); err != nil {
		return delivery.Result{}, fmt.Errorf("spam: decode rspamd reply: %w", err)
	}
